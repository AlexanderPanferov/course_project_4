...
class Vacancy:
    """
    Класс для работы с данными из .json файла
    """
    ...
    def sorting(vacancies):
        """
        Сортирует полученные вакансии исходя из зарплаты от большего к меньшему:
    ...
    def get_top(vacancies, top_count):
        """
        Возвращает полученный топ вакансий по запросу, исходя из зарплаты от большего к меньшему:
        создает .json файл для проверки работы
        """
        top_list = []
        vacancies_sort = sorted(vacancies, key=lambda vacancy: vacancy["salary_from"], reverse=True)
